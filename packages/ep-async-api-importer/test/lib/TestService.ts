...
export class TestService {
  ...
  public static validateFilePathWithReadPermission = (filePath: string) => {
    const fp: string | undefined = CliUtils.validateFilePathWithReadPermission(filePath);
    if(fp === undefined) throw new Error(`filePath is invalid: ${filePath}`);
    return fp;
  }

  public static createTestApiSpecRecordList = async({ apiFileList, overrideApplicationDomainName, overrideAssetApplicationDomainName }:{
  ...
  public static getTestApiSpecRecordList(): Array<TTestApiSpecRecord> { return TestService.testApiSpecRecordList; }
