...
describe(`${scriptName}`, () => {
  ...
  it(`${scriptName}: check app domains`, async () => {
    try {
      ...
      const applicationDomain: ApplicationDomain = await EpSdkApplicationDomainsService.getByName({ applicationDomainName: AsyncApiSpecFile_X_EpApplicationDomainName });
      const eventApisResponse: EventApisResponse = await EventApIsService.getEventApis({ applicationDomainId: applicationDomain.id });
      ...
      let eventsResponse: EventsResponse = await EventsService.getEvents({ applicationDomainId: applicationDomain.id});
      message = `eventsResponse=${JSON.stringify(eventsResponse, null, 2)}`;
      expect(eventsResponse.data.length, message).to.equal(0);
      let schemasResponse: SchemasResponse = await SchemasService.getSchemas({ applicationDomainId: applicationDomain.id});
      ...
      const assetApplicationDomain: ApplicationDomain = await EpSdkApplicationDomainsService.getByName({ applicationDomainName: AsyncApiSpecFile_X_EpAssetApplicationDomainName });
      eventsResponse = await EventsService.getEvents({ applicationDomainId: assetApplicationDomain.id});
      message = `eventsResponse=${JSON.stringify(eventsResponse, null, 2)}`;
      expect(eventsResponse.data.length, message).to.equal(2);
      schemasResponse = await SchemasService.getSchemas({ applicationDomainId: assetApplicationDomain.id});
      message = `schemasResponse=${JSON.stringify(schemasResponse, null, 2)}`;
