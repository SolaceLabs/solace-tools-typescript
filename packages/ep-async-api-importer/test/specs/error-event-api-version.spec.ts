...
const initializeGlobals = () => {
  FileList.push(...CliUtils.createFileList(`${TestConfig.getConfig().dataRootDir}/single-tests/warning-event-api-version.spec.yml`));
  ...
  it(`${scriptName}: should import specs: with error`, async () => {
    try {
      ...
      try {
        ...
        expect(e instanceof CliImporterTestRunAssetsInconsistencyError, `e not instance of CliImporterTestRunAssetsInconsistencyError, message=${e.message}`).to.be.true;
        const ciiImporterTestRunAssetsInconsistencyError: CliImporterTestRunAssetsInconsistencyError = e;
        const details = ciiImporterTestRunAssetsInconsistencyError.details;
        const requestedUpdates = details.requestedUpdates;
        expect(requestedUpdates, 'requestedUpdates is undefined').to.be.not.undefined;
        expect(JSON.stringify(requestedUpdates), 'wrong requestedUpdates').to.include('description');
        expect(JSON.stringify(requestedUpdates), 'wrong requestedUpdates').to.include('version');
      }
      const cliRunSummaryList: Array<ICliRunSummary_Base> = CliRunSummary.getSummaryLogList();
      // // DEBUG
