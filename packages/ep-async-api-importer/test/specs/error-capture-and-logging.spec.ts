...
const SingleTest_SameApiVersionCombinedIssues = "same-api-version-combined-issues";
const SingleTest_SameApiVersionDifferentChannels = "same-api-version-different-channels";
...
const SingleTest_SameApiVersionRemovedChannel = "same-api-version-removed-channel";

let FileList: Array<string> = [];

const initializeGlobals = () => {
  FileList.push(...CliUtils.createFileList(`${TestConfig.getConfig().dataRootDir}/${SpecFilesDir}/**/*.spec.yml`));
...
describe(`${scriptName}`, () => {
  ...
  it(`${scriptName}: should report correct errors for: ${SingleTest_NewEventSameVersion} `, async () => {
    ...
    try {
      CliConfig.getCliImporterManagerOptions().asyncApiFileList = CliUtils.createFileList(`${TestConfig.getConfig().dataRootDir}/${SpecFilesDir}/${SingleTest_NewEventSameVersion}/*.spec.yml`);
      ...
      expect(e instanceof CliError, TestLogger.createNotCliErrorMesssage(e.message)).to.be.true;
      const ce: CliImporterTestRunAssetsInconsistencyError = new CliImporterTestRunAssetsInconsistencyError('check', {});
      ...
      const secondLastEntry: ICliRunSummary_Base = cliRunSummaryList[cliRunSummaryList.length-2];
      expect(secondLastEntry.type, `secondLastEntry.type, wrong type = ${secondLastEntry.type}`).to.equal(ECliRunSummary_Type.VersionObject);
      const versionObjectEntry: ICliRunSummary_Task_VersionObject = secondLastEntry as ICliRunSummary_Task_VersionObject;
      expect(versionObjectEntry.action,`versionObjectEntry.action, wrong action = ${versionObjectEntry.action}`).to.equal(EEpSdkTask_Action.WOULD_FAIL_CREATE_NEW_VERSION_ON_EXACT_VERSION_REQUIREMENT);
    }
  });
  ...
  it(`${scriptName}: should report correct errors for: ${SingleTest_SameApiVersionCombinedIssues} `, async () => {
    ...
    try {
      CliConfig.getCliImporterManagerOptions().asyncApiFileList = CliUtils.createFileList(`${TestConfig.getConfig().dataRootDir}/${SpecFilesDir}/${SingleTest_SameApiVersionCombinedIssues}/*.spec.yml`);
      const cliImporter = new CliImporterManager(CliConfig.getCliImporterManagerOptions());
      const xvoid: void = await cliImporter.run();      
      expect(false, 'should never get here').to.be.true;
    } catch(e) {
      ...
      const thirdLastEntry: ICliRunSummary_Base = cliRunSummaryList[cliRunSummaryList.length-3];
      expect(thirdLastEntry.type, `wrong type = ${thirdLastEntry.type}`).to.equal(ECliRunSummary_Type.EventApiVersioningError);
      const secondLastEntry: ICliRunSummary_Base = cliRunSummaryList[cliRunSummaryList.length-2];
      expect(secondLastEntry.type, `wrong type = ${secondLastEntry.type}`).to.equal(ECliRunSummary_Type.VersionObject);
      const versionObjectEntry: ICliRunSummary_Task_VersionObject = secondLastEntry as ICliRunSummary_Task_VersionObject;
      expect(versionObjectEntry.action,`wrong action = ${versionObjectEntry.action}`).to.equal(EEpSdkTask_Action.WOULD_FAIL_CREATE_NEW_VERSION_ON_EXACT_VERSION_REQUIREMENT);
    }
  });
  ...
  it(`${scriptName}: should report correct errors for: ${SingleTest_SameApiVersionDifferentChannels} `, async () => {
    ...
    try {
      CliConfig.getCliImporterManagerOptions().asyncApiFileList = CliUtils.createFileList(`${TestConfig.getConfig().dataRootDir}/${SpecFilesDir}/${SingleTest_SameApiVersionDifferentChannels}/*.spec.yml`);
      const cliImporter = new CliImporterManager(CliConfig.getCliImporterManagerOptions());
      const xvoid: void = await cliImporter.run();      
      expect(false, 'should never get here').to.be.true;
    } catch(e) {
  ...
  it(`${scriptName}: should report correct errors for: ${SingleTest_SameApiVersionNewEnum} `, async () => {
    ...
    try {
      CliConfig.getCliImporterManagerOptions().asyncApiFileList = CliUtils.createFileList(`${TestConfig.getConfig().dataRootDir}/${SpecFilesDir}/${SingleTest_SameApiVersionNewEnum}/*.spec.yml`);
      const cliImporter = new CliImporterManager(CliConfig.getCliImporterManagerOptions());
      const xvoid: void = await cliImporter.run();      
      expect(false, 'should never get here').to.be.true;
    } catch(e) {
  ...
  it(`${scriptName}: should report correct errors for: ${SingleTest_SameApiVersionRemoveEnum} `, async () => {
    ...
    try {
      CliConfig.getCliImporterManagerOptions().asyncApiFileList = CliUtils.createFileList(`${TestConfig.getConfig().dataRootDir}/${SpecFilesDir}/${SingleTest_SameApiVersionRemoveEnum}/*.spec.yml`);
      const cliImporter = new CliImporterManager(CliConfig.getCliImporterManagerOptions());
      const xvoid: void = await cliImporter.run();      
    } catch(e) {
      expect(false, 'should never get here').to.be.true;
  ...
  it(`${scriptName}: should report correct errors for: ${SingleTest_SameApiVersionRemovedChannel} `, async () => {
    ...
    try {
      CliConfig.getCliImporterManagerOptions().asyncApiFileList = CliUtils.createFileList(`${TestConfig.getConfig().dataRootDir}/${SpecFilesDir}/${SingleTest_SameApiVersionRemovedChannel}/*.spec.yml`);
      const cliImporter = new CliImporterManager(CliConfig.getCliImporterManagerOptions());
      const xvoid: void = await cliImporter.run();      
      expect(false, 'should never get here').to.be.true;
    } catch(e) {
