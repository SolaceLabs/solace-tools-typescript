...
export class CliEventApiImporter extends CliAssetsImporter {
  ...
  private run_present_event_api = async({ applicationDomainId, assetApplicationDomainId, epAsyncApiDocument, checkmode }:{
    ...
    return {
      eventApiId: eventApiId,
      eventApiVersionId: cliEventApiImporterRunPresentEventApiVersionReturn.eventApiVersionId
    };
  ...
  protected generate_assets_ouput = async({ cliImporterGenerateAssetsOptions }:{
    ...
    try {
      ...
      const assetOutputRootDir: string = CliUtils.ensureDirExists(this.cliImporterOptions.assetOutputDir, applicationDomainNameAsFilePath + '/' + apiTitleAsFilePath);
      ...
      for(const [topic, epAsyncApiChannelDocument] of epAsyncApiChannelDocumentMap) {
        ...
        if(epAsynApiChannelPublishOperation !== undefined) {
          ...
          if(epAsyncApiMessageDocument.getContentType() !== E_EpAsyncApiContentTypes.APPLICATION_JSON) throw new CliAsyncApiSpecFeatureNotSupportedError(logName, "unsupported message content type", {
      ...
      cliEventApiImporterGenerateAssetsReturn.error = CliErrorFactory.createCliError({
        logName: logName,
        e: e
      });
    } finally {
  ...
  public async run_import({ cliImporterRunOptions }:{
    ...
    try {
      ...
      const cliEventApiImporterRunPresentReturn: ICliEventApiImporterRunPresentReturn = await this.run_present({
  ...
  public async run({ cliImporterRunOptions }:{
    ...
    const cliEventApiImporterRunImportReturn: ICliEventApiImporterRunImportReturn = await this.run_import({
      cliImporterRunOptions: cliImporterRunOptions,
    });
    ...
    if(cliImporterRunOptions.generateAssetsOutput) {
      const cliEventApiImporterGenerateAssetsReturn: ICliEventApiImporterGenerateAssetsReturn = await this.generate_assets_ouput({ 
