...
export interface ICliEventApiImporterRunImportReturn
  extends ICliAssetsImporterRunReturn {
  apiTitle: string;
  applicationDomainId: string;
  eventApiId: string;
  eventApiVersionId: string;
}
export interface ICliEventApiImporterRunOptions
  extends ICliAssetsImporterRunOptions {
  generateAssetsOutput: boolean;
}
export interface ICliEventApiImporterRunReturn
  extends ICliAssetsImporterRunReturn {
...
export class CliEventApiImporter extends CliAssetsImporter {
  ...
  private run_present_event_api_version = async ({
    ...
    const latestExistingEventApiVersionObjectBefore:
      | EventApiVersion
      | undefined =
    ...
    const epSdkEventApiVersionTask_ExecuteReturn_Check: IEpSdkEventApiVersionTask_ExecuteReturn =
    ...
    CliLogger.trace(
      CliLogger.createLogEntry(logName, {
        code: ECliStatusCodes.IMPORTING_EP_EVENT_API_VERSION_CHECK,
        details: {
    ...
    if (
      epSdkEventApiVersionTask_ExecuteReturn_Check.epSdkTask_TransactionLogData
        .epSdkTask_Action ===
      ...
      // test_mode: error
      if (latestExistingEventApiVersionObjectBefore === undefined)
      ...
      if (CliRunContext.get().runMode === ECliRunContext_RunMode.RELEASE) {
        ...
        if (epSdkEventApiVersionTask_ExecuteReturn.epObject.id === undefined)
          throw new CliEPApiContentError(
            logName,
            "epSdkEventApiVersionTask_ExecuteReturn.epObject.id === undefined",
            {
        ...
        CliLogger.warn(
          CliLogger.createLogEntry(logName, {
            code: ECliStatusCodes.IMPORTING_EP_EVENT_API_WITH_WARNING,
            details: {
              warning: [
        ...
        CliRunSummary.processedEventApiVersionWithWarning({
          ...
          requestedUpdates:
            epSdkEventApiVersionTask_ExecuteReturn_Check
              .epSdkTask_TransactionLogData.epSdkTask_IsUpdateRequiredFuncReturn
              ?.difference,
        });
      } else {
        // issue error
        const requestedUpdates: any =
          await CliRunExecuteReturnLog.getDeepRequestedUpdates(
            epSdkEventApiVersionTask_ExecuteReturn_Check
          );
  ...
  private run_present_event_api = async ({
    ...
    CliLogger.debug(
      CliLogger.createLogEntry(logName, {
        code: ECliStatusCodes.IMPORTING_START_API,
        details: {},
      })
    );
    ...
    const epSdkEventApiTask = new EpSdkEventApiTask({
      ...
      eventApiObjectSettings: {
        shared: true,
        brokerType: this.cliImporterOptions
          .cliAssetImport_BrokerType as unknown as EventApi.brokerType,
      },
      epSdkTask_TransactionConfig: this.get_IEpSdkTask_TransactionConfig(),
      checkmode: checkmode,
    });
    ...
    /* istanbul ignore next */
    if (epSdkEventApiTask_ExecuteReturn.epObject.id === undefined)
      throw new CliEPApiContentError(
        logName,
        "epSdkEventApiTask_ExecuteReturn.epObject.id === undefined",
        {
          epSdkEventApiTask_ExecuteReturn: epSdkEventApiTask_ExecuteReturn,
        }
      );
    ...
    const cliEventApiImporterRunPresentEventApiVersionReturn: ICliEventApiImporterRunPresentEventApiVersionReturn =
  ...
  protected async run_present({
    ...
    const apiTitle: string =
      cliImporterRunPresentOptions.epAsyncApiDocument.getTitle();
    const apiVersion: string =
      cliImporterRunPresentOptions.epAsyncApiDocument.getVersion();
    const epApplicationDomainName: string =
      cliImporterRunPresentOptions.epAsyncApiDocument.getApplicationDomainName();
    const epAssetApplicationDomainName: string =
    ...
    CliLogger.trace(
      CliLogger.createLogEntry(logName, {
        code: ECliStatusCodes.IMPORTING_EP_APPLICATION_DOMAIN,
    ...
    /* istanbul ignore next */
    if (epSdkApplicationDomainTask_ExecuteReturn.epObject.id === undefined)
      throw new CliEPApiContentError(
        logName,
        "epSdkApplicationDomainTask_ExecuteReturn.epObject.id === undefined",
        {
          applicationDomainObject:
            epSdkApplicationDomainTask_ExecuteReturn.epObject,
        }
      );

    // present event api
    const cliEventApiImporterRunPresentEventApiReturn: ICliEventApiImporterRunPresentEventApiReturn =
      await this.run_present_event_api({
        applicationDomainId:
    ...
    const cliEventApiImporterRunPresentReturn: ICliEventApiImporterRunPresentReturn =
      {
        ...
        eventApiVersionId:
          cliEventApiImporterRunPresentEventApiReturn.eventApiVersionId,
      };
    CliRunContext.pop();
  ...
  public async run_import({
    ...
    try {
      ...
      cliEventApiImporterRunImportReturn.apiTitle =
        epAsyncApiDocument.getTitle();
      cliEventApiImporterRunImportReturn.applicationDomainName =
        epAsyncApiDocument.getApplicationDomainName();
      cliEventApiImporterRunImportReturn.assetApplicationDomainName =
      ...
      cliEventApiImporterRunImportReturn.applicationDomainId =
        cliEventApiImporterRunPresentReturn.applicationDomainId;
      cliEventApiImporterRunImportReturn.eventApiId =
        cliEventApiImporterRunPresentReturn.eventApiId;
      cliEventApiImporterRunImportReturn.eventApiVersionId =
        cliEventApiImporterRunPresentReturn.eventApiVersionId;

      CliLogger.debug(
        CliLogger.createLogEntry(logName, {
          code: ECliStatusCodes.IMPORTING_DONE_API,
          details: {
            cliEventApiImporterRunPresentReturn:
              cliEventApiImporterRunPresentReturn,
          },
        })
      );
    } catch (e: any) {
      cliEventApiImporterRunImportReturn.error = CliErrorFactory.createCliError(
        {
          logName: logName,
          e: e,
        }
      ...
      if (cliEventApiImporterRunImportReturn.error !== undefined) {
        CliLogger.error(
          CliLogger.createLogEntry(logName, {
            code: ECliStatusCodes.IMPORTING_ERROR_API,
            details: {
  ...
  public async run({
    ...
    if (cliImporterRunOptions.generateAssetsOutput) {
      const cliEventApiImporterGenerateAssetsReturn: ICliEventApiImporterGenerateAssetsReturn =
        await this.generate_assets_ouput({
          cliImporterGenerateAssetsOptions: {
