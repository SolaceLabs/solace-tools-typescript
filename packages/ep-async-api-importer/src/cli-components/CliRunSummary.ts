...
export interface ICliRunSummary_ApiChannel_Operation extends ICliRunSummary_Base {
...
export interface ICliRunSummary_Task_AssetApplicationDomain extends ICliRunSummary_Task {
...
Start Run: ${cliRunSummary_StartRun.runMode} ------------------------
  ...
  public setUpTestDomains = ({ cliRunSummary_SetupTestDomains }:{
    cliRunSummary_SetupTestDomains: ICliRunSummary_SetupTestDomains;
  }): void => {
    const consoleOutput = `
  Setting up Test Domain(s) ...
    `;
    this.log(ECliSummaryStatusCodes.SETUP_TEST_DOMAINS, cliRunSummary_SetupTestDomains, consoleOutput);
  }

  public setUpTestApi = ({ cliRunSummary_SetupTestApi }:{
    ...
    `;
    this.log(ECliSummaryStatusCodes.SETUP_TEST_API, cliRunSummary_SetupTestApi, consoleOutput);
  }
  ...
  public generatingApiOutput = ({ cliRunSummary_GenerateApiOutput }: {
    ...
    `;
    this.log(ECliSummaryStatusCodes.GENERATING_API_OUTPUT, this.addRun(cliRunSummary_GenerateApiOutput), consoleOutput);
  }
...
>>> Processing Api: ${cliRunSummary_Api.apiName}@${cliRunSummary_Api.apiVersion}
  ...
  public processedAssetApplicationDomain = ({ epSdkAssetApplicationDomainTask_ExecuteReturn }:{
    epSdkAssetApplicationDomainTask_ExecuteReturn: IEpSdkApplicationDomainTask_ExecuteReturn
  }): void => {
    this.assetApplicationDomainName = epSdkAssetApplicationDomainTask_ExecuteReturn.epObject.name;
    const consoleOutput = `
    ...
    `;
    const cliRunSummary_Task_AssetApplicationDomain: Required<Omit<ICliRunSummary_Task_AssetApplicationDomain, "runMode">> = {
      type: ECliRunSummary_Type.AssetApplicationDomain,
      applicationDomainName: this.assetApplicationDomainName,
      action: epSdkAssetApplicationDomainTask_ExecuteReturn.epSdkTask_TransactionLogData.epSdkTask_Action,
    }
    this.log(ECliSummaryStatusCodes.PROCESSED_ASSET_APPLICATION_DOMAIN, this.addTaskElements(cliRunSummary_Task_AssetApplicationDomain), consoleOutput);
  }
  ...
  public processedEventApiVersionWithError = ({ targetEventApiVersion, targetEventApiState, epSdkEventApiVersionTask_ExecuteReturn, latestExistingEventApiVersionObjectBefore, requestedUpdates }: { 
    ...
    const cliRunSummary_Task_VersionObject_Error: ICliRunSummary_Task_VersionObject_Error = {
      ...
      newVersion: epSdkEventApiVersionTask_ExecuteReturn.epObject.version ? epSdkEventApiVersionTask_ExecuteReturn.epObject.version : 'undefined',
      newVersionState: epSdkEventApiVersionTask_ExecuteReturn.epObject.stateId ? epSdkEventApiVersionTask_ExecuteReturn.epObject.stateId : 'undefined',
    ...
    this.log(ECliSummaryStatusCodes.PROCESSED_EVENT_API_VERSION, this.addTaskElements(cliRunSummary_Task_VersionObject_Error), consoleOutput);
    this.processedVersionObject(ECliSummaryStatusCodes.PROCESSED_EVENT_API_VERSION, epSdkEventApiVersionTask_ExecuteReturn);
  }

  public processedEventApiVersionWithWarning = ({ targetEventApiVersion, targetEventApiState, epSdkEventApiVersionTask_ExecuteReturn, latestExistingEventApiVersionObjectBefore, requestedUpdates }: { 
    targetEventApiVersion: string;
    targetEventApiState: string;
    epSdkEventApiVersionTask_ExecuteReturn: IEpSdkEventApiVersionTask_ExecuteReturn;
    latestExistingEventApiVersionObjectBefore: EventApiVersion;
    ...
    this.log(ECliSummaryStatusCodes.PROCESSED_EVENT_API_VERSION, this.addTaskElements(cliRunSummary_Task_VersionObject_Warning), consoleOutput);
    this.processedVersionObject(ECliSummaryStatusCodes.PROCESSED_EVENT_API_VERSION, epSdkEventApiVersionTask_ExecuteReturn);
  }
  ...
  public createImportSummary = (cliImporterManagerMode: ECliImporterManagerMode): ICliImportSummary => {
    ...
    const cliRunSummary_AssetApplicationDomain_List: Array<ICliRunSummary_Task_AssetApplicationDomain> = cliRunSummary_LogBase_Filtered_List.filter( (cliRunSummary_LogBase: ICliRunSummary_LogBase) => {
      return cliRunSummary_LogBase.type === ECliRunSummary_Type.AssetApplicationDomain;
    }) as unknown as Array<ICliRunSummary_Task_AssetApplicationDomain>;
    const processedAssetApplicationDomains = cliRunSummary_AssetApplicationDomain_List.length;
    const createdAssetApplicationDomains = cliRunSummary_AssetApplicationDomain_List.reduce( (count, item) => count + Number(item.type === ECliRunSummary_Type.AssetApplicationDomain && item.action !== EEpSdkTask_Action.NO_ACTION), 0);
    ...
    const cliRunSummary_LogBase_TestPass_1_FilteredList: Array<ICliRunSummary_LogBase> = cliRunSummary_LogBase_List.filter( (cliRunSummary_LogBase: ICliRunSummary_LogBase) => {
      return cliRunSummary_LogBase.runMode === ECliRunContext_RunMode.TEST_PASS_1;
    });
    const cliRunSummary_CreatedVersionObjectError_List: Array<ICliRunSummary_Task_VersionObject_Error> = cliRunSummary_LogBase_TestPass_1_FilteredList.filter( (cliRunSummary_LogBase: ICliRunSummary_LogBase) => {
      return cliRunSummary_LogBase.type === ECliRunSummary_Type.EventApiVersioningError;
    ...
    const createdEventApiVersions = cliRunSummary_CreatedVersionObject_List.reduce( (count, item) => count + Number(item.epObjectType === EEpSdkObjectTypes.EVENT_API_VERSION), 0);
    const createdApplicationVersions = cliRunSummary_CreatedVersionObject_List.reduce( (count, item) => count + Number(item.epObjectType === EEpSdkObjectTypes.APPLICATION_VERSION), 0);
    const createdEventVersions = cliRunSummary_CreatedVersionObject_List.reduce( (count, item) => count + Number(item.epObjectType === EEpSdkObjectTypes.EVENT_VERSION), 0);
    const createdSchemaVersions = cliRunSummary_CreatedVersionObject_List.reduce( (count, item) => count + Number(item.epObjectType === EEpSdkObjectTypes.SCHEMA_VERSION), 0);
    const createdEnumVersions = cliRunSummary_CreatedVersionObject_List.reduce( (count, item) => count + Number(item.epObjectType === EEpSdkObjectTypes.ENUM_VERSION), 0);
...
Import Summary for mode: ${cliImporterManagerOptions.cliImporterManagerMode}
  ...
  Processed Asset Application Domains: ${cliImportSummary.processedAssetApplicationDomains}
    Created Asset Application Domains: ${cliImportSummary.createdAssetApplicationDomains}
...
------------------------------------------------------------------------------------------------    
${cliImportSummary.errors.length > 0 ? JSON.stringify(cliImportSummary.errors, null, 2) : ''}
------------------------------------------------------------------------------------------------    
    `;
    this.log(ECliSummaryStatusCodes.IMPORT_SUMMARY, cliImportSummary, consoleOutput, true);
  }
