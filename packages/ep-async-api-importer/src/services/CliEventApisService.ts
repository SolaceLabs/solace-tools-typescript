...
class CliEventApisService {
  public deepCopyLatestEventApiVersion = async ({
    epAsyncApiDocument,
  }: {
    ...
    if (fromApplicationDomain !== undefined) {
      ...
      if (fromApplicationDomain.id === undefined)
        throw new CliEPApiContentError(
          logName,
          "fromApplicationDomain.id === undefined",
          {
      ...
      if (fromAssetsApplicationDomainName !== fromApplicationDomainName) {
        const fromAssetsApplicationDomain: ApplicationDomain | undefined =
          await EpSdkApplicationDomainsService.getByName({
      ...
      const toEpSdkApplicationDomainTask_ExecuteReturn: IEpSdkApplicationDomainTask_ExecuteReturn =
        await toApplicationDomainsTask.execute();
      /* istanbul ignore next */
      if (toEpSdkApplicationDomainTask_ExecuteReturn.epObject.id === undefined)
        throw new CliEPApiContentError(
          logName,
          "toEpSdkApplicationDomainTask_ExecuteReturn.epObject.id === undefined",
          {
            applicationDomainObject:
              toEpSdkApplicationDomainTask_ExecuteReturn.epObject,
          }
        );
      toApplicationDomainId =
        toEpSdkApplicationDomainTask_ExecuteReturn.epObject.id;
      const toAssetsApplicationDomainsTask = new EpSdkApplicationDomainTask({
        epSdkTask_TargetState: EEpSdkTask_TargetState.PRESENT,
        applicationDomainName: toAssetsApplicationDomainName,
        applicationDomainSettings: {},
      });
      const toAssetsEpSdkApplicationDomainTask_ExecuteReturn: IEpSdkApplicationDomainTask_ExecuteReturn =
        await toAssetsApplicationDomainsTask.execute();
      /* istanbul ignore next */
      if (
        toAssetsEpSdkApplicationDomainTask_ExecuteReturn.epObject.id ===
        undefined
      )
        throw new CliEPApiContentError(
          logName,
          "toAssetsEpSdkApplicationDomainTask_ExecuteReturn.epObject.id === undefined",
          {
            applicationDomainObject:
              toAssetsEpSdkApplicationDomainTask_ExecuteReturn.epObject,
          }
        );
      toAssetsApplicationDomainId =
        toAssetsEpSdkApplicationDomainTask_ExecuteReturn.epObject.id;

      const eventApiVersion: EventApiVersion | undefined =
