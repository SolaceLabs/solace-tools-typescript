...
async function main() {
  ...
  xvoid;
  CliLogger.trace(CliLogger.createLogEntry(logName, { code: ECliStatusCodes.INFO, message: 'finished.', details: {
    cliConfig: CliConfig.getCliConfig()
  }}));
}
...
function getCommandLineOptionValues(): OptionValues {
  ...
  const assetDomainOption: Option = Program.createOption('-ad, --assetDomain <value>', 'Asset Application Domain Name. Overrides the asset application domain name extracted from each in Async API file, path=$.x-ep-assets-application-domain-name.');
