...
const FunkyApplicationDomainName = "Developer Portal Showcase/consumer-apps";
let FunkyApplicationDomainId: string | undefined;
...
describe(`${scriptName}`, () => {
  ...
  after(async () => {
    TestContext.newItId();
    try {
      await EpSdkApplicationDomainsService.deleteById({ xContextId: 'xContextId', applicationDomainId: ApplicationDomainId });
    } catch (e) {}
    try {
      await EpSdkApplicationDomainsService.deleteById({ xContextId: 'xContextId', applicationDomainId: FunkyApplicationDomainId });
    } catch (e) {}
  });
  ...
  it(`${scriptName}: ensure application domains are absent`, async () => {
    TestContext.newItId();
    try {
      await EpSdkApplicationDomainsService.deleteByName({ xContextId: 'xContextId', applicationDomainName: ApplicationDomainName });
    } catch (e) {}
    try {
      await EpSdkApplicationDomainsService.deleteByName({ xContextId: 'xContextId', applicationDomainName: FunkyApplicationDomainName });
    } catch (e) {}
    // // DEBUG: 
    // expect(false, 'deleted?').to.be.true;
  });

  it(`${scriptName}: application domain present: test funky name`, async () => {
    try {
      const epSdkApplicationDomainTask = new EpSdkApplicationDomainTask({
        epSdkTask_TargetState: EEpSdkTask_TargetState.PRESENT,
        applicationDomainName: FunkyApplicationDomainName,
      });
      ...
      FunkyApplicationDomainId = epSdkApplicationDomainTask_ExecuteReturn.epObject.id;
