...
describe(`${scriptName}`, () => {
  ...
  it(`${scriptName}: event api present: checkmode create`, async () => {
    try {
      ...
      const message = TestLogger.createLogMessage(
        "epSdkEventApiTask_ExecuteReturn",
        epSdkEventApiTask_ExecuteReturn
      );

      expect(
        epSdkEventApiTask_ExecuteReturn.epSdkTask_TransactionLogData
          .epSdkTask_Action,
        message
  ...
  it(`${scriptName}: event api present: create: idempotency`, async () => {
    try {
      ...
      expect(epSdkEventApiTask_ExecuteReturn.epObject.id, message).to.eq(
        EventApiId
      );
  ...
  it(`${scriptName}: event api present: create with brokerType = kafka`, async () => {
    try {
      ...
      expect(
        epSdkEventApiTask_ExecuteReturn.epObject.brokerType,
        message
      ).to.eq(EventApi.brokerType.KAFKA);
