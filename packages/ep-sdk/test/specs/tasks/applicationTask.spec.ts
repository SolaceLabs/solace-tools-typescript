...
describe(`${scriptName}`, () => {
  ...
  it(`${scriptName}: application present: checkmode create`, async () => {
    try {
      ...
      const epSdkApplicationTask_ExecuteReturn: IEpSdkApplicationTask_ExecuteReturn =
        await epSdkApplicationTask.execute();
      ...
      expect(
        epSdkApplicationTask_ExecuteReturn.epSdkTask_TransactionLogData
          .epSdkTask_Action,
        message
  ...
  it(`${scriptName}: application present: create: nothing to do`, async () => {
    try {
      ...
      expect(epSdkApplicationTask_ExecuteReturn.epObject.id, message).to.eq(
        ApplicationId
      );
  ...
  it(`${scriptName}: application present: update: should throw error`, async () => {
    try {
      ...
      expect(
        e instanceof EpSdkEpApiError,
        TestLogger.createNotApiErrorMessage(e)
      ).to.be.true;
      const epSdkEpApiError: EpSdkEpApiError = e;
      expect(
        epSdkEpApiError.apiError.status,
        TestLogger.createApiTestFailMessage("failed")
      ).to.eq(400);
      expect(
        JSON.stringify(epSdkEpApiError.apiError.body),
        TestLogger.createApiTestFailMessage("failed")
      ).to.contain("immutable field");
    }
  });
