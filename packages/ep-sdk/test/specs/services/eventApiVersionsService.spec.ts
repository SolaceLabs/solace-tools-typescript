...
describe(`${scriptName}`, () => {
  ...
  it(`${scriptName}: should get event api version by version`, async () => {
    try {
      const eventApiVersion: EventApiVersion =
        await EpSdkEventApiVersionsService.getVersionByVersion({
          eventApiId: EventApiId,
          eventApiVersionString: EventApiVersionString,
        });
      expect(
        eventApiVersion.version,
        TestLogger.createApiTestFailMessage("version mismatch")
      ).to.eq(EventApiVersionString);
    } catch (e) {
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
      expect(e instanceof EpSdkError, TestLogger.createNotEpSdkErrorMessage(e))
  ...
  it(`${scriptName}: should get event api versions for event api id`, async () => {
    try {
      const eventApiVersionList: Array<EventApiVersion> =
        await EpSdkEventApiVersionsService.getVersionsForEventApiId({
          eventApiId: EventApiId,
        });
      expect(
        eventApiVersionList.length,
  ...
  it(`${scriptName}: should get event api versions for event api name`, async () => {
    try {
      const eventApiVersionList: Array<EventApiVersion> =
        await EpSdkEventApiVersionsService.getVersionsForEventApiName({
          applicationDomainId: ApplicationDomainId,
  ...
  it(`${scriptName}: should create new event api version`, async () => {
    try {
      ...
      const createdEventVersion: EventVersion =
        await TestHelpers.createEventVersion({
          applicationDomainId: ApplicationDomainId,
      ...
      expect(created.stateId, "created.stateId failed").to.equal(
        EpSdkStatesService.releasedId
      );
    } catch (e) {
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
  ...
  it(`${scriptName}: should get latest version string`, async () => {
    try {
      ...
      ).to.eq(EventApiNextVersionString);
    } catch (e) {
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
      expect(e instanceof EpSdkError, TestLogger.createNotEpSdkErrorMessage(e))
  ...
  it(`${scriptName}: should get latest version for event api name`, async () => {
    try {
      ...
      expect(
        eventApiVersion,
        TestLogger.createApiTestFailMessage("eventApiVersion === undefined")
      ).to.not.be.undefined;
      expect(
        eventApiVersion.version,
        TestLogger.createApiTestFailMessage("version string mismatch")
  ...
  it(`${scriptName}: should catch displayName validation error`, async () => {
    const DisplayName =
      "very, very, very, very, very, very, very, very, very, very, very, very, very long display name";
    try {
      const create: EventApiVersion = {
        eventApiId: EventApiId,
        description: `event api version for event = ${EventApiName}, id=${EventApiId}`,
      ...
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
      expect(
        e instanceof EpSdkValidationError,
        TestLogger.createNotEpSdkErrorMessage(e)
      ).to.be.true;
      const epSdkValidationError: EpSdkValidationError = e;
      expect(
        epSdkValidationError.toString(),
        TestLogger.createEpSdkTestFailMessage(
          `error does not contain ${DisplayName}`,
          epSdkValidationError
        )
      ).to.contain(DisplayName);
    }
  });
  ...
  it(`${scriptName}: should create 10 event api versions & get latest version string them using paging`, async () => {
    ...
    try {
      ...
      for (let i = 0; i < VersionQuantity; i++) {
        VersionString = `3.0.${i}`;
        const versionResponse: EventApiVersionResponse =
          await EventApIsService.createEventApiVersion({
            requestBody: {
              eventApiId: EventApiId,
      ...
      let latestObjectVersion: EventApiVersion =
        await EpSdkEventApiVersionsService.getLatestVersionForEventApiId({
          eventApiId: EventApiId,
          applicationDomainId: ApplicationDomainId,
        });
  ...
  it(`${scriptName}: should create 2 event apis with 1 version each & get event and version list`, async () => {
    ...
    try {
      {
        const eventApiResponse_1: EventApiResponse =
          await EventApIsService.createEventApi({
            requestBody: {
              applicationDomainId: ApplicationDomainId,
              name: "EventApi_1",
        ...
        const eventApiVersionResponse_1: EventApiVersionResponse =
          await EventApIsService.createEventApiVersion({
            requestBody: {
      ...
      {
        const eventApiResponse_2: EventApiResponse =
          await EventApIsService.createEventApi({
            requestBody: {
              applicationDomainId: ApplicationDomainId,
              name: "EventApi_2",
        ...
        const eventApiVersionResponse_2: EventApiVersionResponse =
          await EventApIsService.createEventApiVersion({
            requestBody: {
      ...
      const epSdkEventApiAndVersionList: Array<EpSdkEventApiAndVersion> =
        await EpSdkEventApiVersionsService.getObjectAndVersionListForEventApiVersionIds(
          {
      ...
      expect(
        epSdkEventApiAndVersionList[0].eventApi.id,
        "wrong event api id"
      ).to.equal(EventApiId_1);
      expect(
        epSdkEventApiAndVersionList[0].eventApiVersion.id,
        "wrong event api version id"
      ).to.equal(EventApiVersionId_1);

      expect(
        epSdkEventApiAndVersionList[1].eventApi.id,
        "wrong event api id"
      ).to.equal(EventApiId_2);
      expect(
        epSdkEventApiAndVersionList[1].eventApiVersion.id,
        "wrong event api version id"
      ).to.equal(EventApiVersionId_2);
    } catch (e) {
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
      expect(e instanceof EpSdkError, TestLogger.createNotEpSdkErrorMessage(e))
