...
describe(`${scriptName}`, () => {
  ...
  it(`${scriptName}: should set VersionCustomAttribute on application version`, async () => {
    try {
      const applicationVersion: ApplicationVersion = await EpSdkApplicationVersionsService.setCustomAttributes({
        applicationVersionId: ApplicationVersionId,
        epSdkCustomAttributes: [VersionCustomAttribute],
      })
      const customAttributes = applicationVersion.customAttributes;
  ...
  it(`${scriptName}: should create new application version`, async () => {
    try {
      ...
      const created: ApplicationVersion =
  ...
  it(`${scriptName}: should get latest version string`, async () => {
    try {
      ...
      ).to.eq(ApplicationNextVersionString);
    } catch (e) {
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
      expect(e instanceof EpSdkError, TestLogger.createNotEpSdkErrorMessage(e))
  ...
  it(`${scriptName}: should get latest version for application name`, async () => {
    try {
      ...
      expect(
        applicationVersion,
        TestLogger.createApiTestFailMessage("applicationVersion === undefined")
      ).to.not.be.undefined;
      expect(
        applicationVersion.version,
        TestLogger.createApiTestFailMessage("version string mismatch")
  ...
  it(`${scriptName}: should create 10 application versions & get latest version string them using paging`, async () => {
    ...
    try {
      ...
      const versionList: Array<ApplicationVersion> =
        await EpSdkApplicationVersionsService.getVersionsForApplicationId({
          applicationId: ApplicationId,
        });
      expect(
        versionList.length,
      ...
      let latestObjectVersion: ApplicationVersion =
        await EpSdkApplicationVersionsService.getLatestVersionForApplicationId({
          applicationId: ApplicationId,
          applicationDomainId: ApplicationDomainId,
        });
