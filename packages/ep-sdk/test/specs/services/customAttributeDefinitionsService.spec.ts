...
const CustomAttributeDefinition_1_UpdateValue =
  "CustomAttributeDefinition_1_UpdateValue";
...
describe(`${scriptName}`, () => {
  before(async () => {
    ...
    if (customAttributeDefinition !== undefined)
      CustomAttributeDefinition_1_Id = customAttributeDefinition.id;
    for (const customAttributeName of CustomAttributeDefinition_NameList) {
      ...
      if (customAttributeDefinition !== undefined)
        CustomAttributeDefinition_IdList.push(customAttributeDefinition.id);
    }
    try {
  ...
  it(`${scriptName}: should get custom attribute definition by name`, async () => {
    try {
      ...
      expect(customAttributeDefinition.associatedEntityTypes, "").to.include(
  ...
  it(`${scriptName}: should get enum with attribute / value`, async () => {
    try {
      ...
      expect(
        JSON.stringify(enumResponse.data.customAttributes),
        `enum does not include attribute name = ${CustomAttributeDefinition_1_Name}`
      ).to.include(CustomAttributeDefinition_1_Name);
      expect(
        JSON.stringify(enumResponse.data.customAttributes),
        `enum does not include attribute value = ${CustomAttributeDefinition_1_Value}`
      ).to.include(CustomAttributeDefinition_1_Value);
    } catch (e) {
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
      expect(e instanceof EpSdkError, TestLogger.createNotEpSdkErrorMessage(e))
  ...
  it(`${scriptName}: should update attribute value for enum`, async () => {
    try {
      ...
      expect(
        JSON.stringify(enumResponse.data.customAttributes),
        `enum does not include attribute value = ${CustomAttributeDefinition_1_UpdateValue}`
      ).to.include(CustomAttributeDefinition_1_UpdateValue);
    } catch (e) {
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
      expect(e instanceof EpSdkError, TestLogger.createNotEpSdkErrorMessage(e))
  ...
  it(`${scriptName}: should remove attribute value for enum`, async () => {
    try {
      ...
      expect(
        enumResponse.data.customAttributes.length,
        `enum still has attributes`
      ).to.equal(0);
    } catch (e) {
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
  ...
  it(`${scriptName}: should remove eum from custom attribute definition associations`, async () => {
    try {
      // ensure it exists
      const before: CustomAttributeDefinition =
        await EpSdkCustomAttributeDefinitionsService.getByName({
          attributeName: CustomAttributeDefinition_1_Name,
        });
      expect(JSON.stringify(before.associatedEntityTypes)).to.include(
        EEpSdkCustomAttributeEntityTypes.ENUM
      );
      // remove enum type without deleting
      const after: CustomAttributeDefinition | undefined =
      ...
      expect(JSON.stringify(before.associatedEntityTypes)).to.not.include(
        `"EEpSdkCustomAttributeEntityTypes.ENUM"`
      );
    } catch (e) {
      if (e instanceof ApiError)
  ...
  it(`${scriptName}: should remove all associations from custom attribute definition and delete it`, async () => {
    try {
      ...
      if (before.associatedEntityTypes === undefined)
        throw new Error("before.associatedEntityTypes");
      ...
      for (const associatedEntityType of before.associatedEntityTypes) {
          ...
          await EpSdkCustomAttributeDefinitionsService.removeAssociatedEntityTypeFromCustomAttributeDefinition(
            {
              ...
              associatedEntityType:
                associatedEntityType as EEpSdkCustomAttributeEntityTypes,
            }
          );
        ...
        if (
          associatedEntityType ===
          before.associatedEntityTypes[before.associatedEntityTypes.length - 1]
        ) {
          expect(
            after,
          ...
          expect(after).to.not.be.undefined;
          expect(JSON.stringify(after.associatedEntityTypes)).to.not.include(
            `"${associatedEntityType}"`
          );
        }
      }
      ...
      expect(after, `after=\n${JSON.stringify(after, null, 2)}`).to.be
        .undefined;
    } catch (e) {
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
