...
describe(`${scriptName}`, () => {
  ...
  it(`${scriptName}: should get enum by name`, async () => {
    try {
      const epEnum: TopicAddressEnum | undefined =
      ...
      expect(
        epEnum,
        TestLogger.createApiTestFailMessage("epEnum === undefined")
      ).to.not.be.undefined;
    } catch (e) {
      if (e instanceof ApiError)
        expect(false, TestLogger.createApiTestFailMessage("failed")).to.be.true;
  ...
  it(`${scriptName}: should get enum by id`, async () => {
    try {
      const epEnum: TopicAddressEnum = await EpSdkEnumsService.getById({
        applicationDomainId: ApplicationDomainId,
        enumId: EnumId,
      });
      expect(
        epEnum.id,
        TestLogger.createApiTestFailMessage(`epEnum.id !== ${EnumId}`)
      ).to.eq(EnumId);
      expect(
        epEnum.applicationDomainId,
  ...
  it(`${scriptName}: should delete enum by id`, async () => {
    try {
      const epEnum: TopicAddressEnum = await EpSdkEnumsService.deleteById({
        applicationDomainId: ApplicationDomainId,
        enumId: EnumId,
      });
      expect(
  ...
  it(`${scriptName}: should delete enum by name`, async () => {
    try {
      const epEnum: TopicAddressEnum = await EpSdkEnumsService.deleteByName({
        applicationDomainId: ApplicationDomainId,
        enumName: EnumName,
      });
      expect(
        epEnum.name,
        TestLogger.createApiTestFailMessage(`epEnum.name !== ${EnumName}`)
      ).to.eq(EnumName);
      expect(
        epEnum.id,
        TestLogger.createApiTestFailMessage(`epEnum.id !== ${EnumId}`)
