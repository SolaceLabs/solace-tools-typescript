...
describe(`${scriptName}`, () => {
  ...
  it(`${scriptName}: should get all messaging services`, async () => {
    try {
      ...
      if (messagingServicesResponse.data === undefined)
        throw new Error("messagingServicesResponse.data === undefined");
  ...
  it(`${scriptName}: should patch all messaging services`, async () => {
    try {
      for (const messagingService of MessagingServiceList) {
        ...
        if (messagingService.id === undefined)
          throw new Error(`messagingService.id === undefined, \n${errMsg}`);
        if (messagingService.messagingServiceConnections) {
          for (const messagingServiceConnection of messagingService.messagingServiceConnections) {
            if (
              messagingServiceConnection.messagingServiceAuthentications ===
              undefined
            )
              throw new Error(
                `messagingServiceConnection.messagingServiceAuthentications === undefined, \n${errMsg}`
              );
            if (
              messagingServiceConnection.messagingServiceAuthentications
                .length !== 0
              ...
              if (
                ...
                throw new Error(
                  `messagingServiceConnection.messagingServiceAuthentications.length !== 1, \n${errMsg}`
                );
              if (
                messagingServiceConnection.messagingServiceAuthentications[0]
                  .authenticationDetails === undefined
              )
                throw new Error(
                  `messagingServiceConnection.messagingServiceAuthentications[0].authenticationDetails === undefined, \n${errMsg}`
                );

              const protocol: string | undefined = getProtocol(
                messagingServiceConnection.messagingServiceAuthentications[0]
                  .authenticationDetails
              );
              if (protocol !== undefined) {
                messagingServiceConnection.protocol = protocol;
                messagingServiceConnection.protocolVersion =
        ...
        const messagingServiceResponse: MessagingServiceResponse =
          await MessagingServicesService.updateMessagingService({
            id: messagingService.id,
            requestBody: {
              name: messagingService.name,
              messagingServiceConnections:
                messagingService.messagingServiceConnections,
            },
          });
        // // DEBUG
