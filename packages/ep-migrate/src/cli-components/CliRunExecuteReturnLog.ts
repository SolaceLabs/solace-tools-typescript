...
class CliRunExecuteReturnLog {
  ...
  private async getEventVersionRequestedUpdates(
        ...
        if (key.includes("schemaVersionId")) {
          ...
          const cliRunExecuteReturnLogEntry:
            ...
            (x: ICliRunExecuteReturnLogEntry) => {
              if (
                ...
                EEpSdkObjectTypes.SCHEMA_VERSION
              ) {
          ...
          if (cliRunExecuteReturnLogEntry === undefined) {
            ...
            const schemaVersion: any = {
              displayName:
                epSdkSchemaVersionTask_ExecuteReturn.epObject.displayName,
          ...
          const cliRunExecuteReturnLogEntry:
            ...
            (x: ICliRunExecuteReturnLogEntry) => {
              if (
                x.epSdkTask_ExecuteReturn.epObjectKeys.epObjectType ===
                EEpSdkObjectTypes.ENUM_VERSION
              ) {
                const epVersionObjectKeys: IEpSdkVersionTask_EpObjectKeys = x
                  .epSdkTask_ExecuteReturn.epSdkTask_TransactionLogData
                  .epObjectKeys as IEpSdkVersionTask_EpObjectKeys;
          ...
          if (cliRunExecuteReturnLogEntry === undefined) {
            ...
            const topicAddressEnumVersionResponse: TopicAddressEnumVersionResponse =
              await EnumsService.getEnumVersion({
                versionId: fromTo.from,
              });
            /* istanbul ignore next */
            if (topicAddressEnumVersionResponse.data === undefined)
              throw new CliEPApiContentError(
                logName,
                "topicAddressEnumVersionResponse.data === undefined",
                {
                  topicAddressEnumVersionResponse:
                    topicAddressEnumVersionResponse,
                }
            ...
            const enumVersion: any = {
              displayName:
                epSdkEnumVersionTask_ExecuteReturn.epObject.displayName,
  ...
  private async getEventVersionRemovedRequestedUpdates(
    ...
    if (cliRunExecuteReturnLogEntry === undefined) {
      // get if from the platform
      const eventVersionResponse: EventVersionResponse =
        await EventsService.getEventVersion({
          id: eventVersionId,
        });
      /* istanbul ignore next */
      if (eventVersionResponse.data === undefined)
        throw new CliEPApiContentError(
          logName,
          "eventVersionResponse.data === undefined",
          {
    ...
    return {
      removed: {
        displayName:
          epSdkEnumVersionTask_ExecuteReturn.epSdkTask_TransactionLogData
            .epSdkTask_GetFuncReturn?.epObject.displayName,
        version:
          epSdkEnumVersionTask_ExecuteReturn.epSdkTask_TransactionLogData
            .epSdkTask_GetFuncReturn?.epObject.version,
      },
    };
  }
